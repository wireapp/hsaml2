...
instance XP.XmlPickler AdditionalMetadataLocation where
  xpickle =
    xpElem "AdditionalMetadataLocation" $
      [XP.biCase|
    ...
    where
      xpAnyURIList =
        XP.xpWrapEither
          ...
          $ XP.xpTextDT
          $ XPS.scDT (namespaceURIString ns) "anyURIListType" []
...
instance XP.XmlPickler AttributeConsumingService where
  xpickle =
    xpElem "AttributeConsumingService" $
      [XP.biCase|
